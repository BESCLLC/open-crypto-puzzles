...
decoded cipher; the fourth is a decorative dead end. Seed words 1 to 5 are printed in the
open on one East page, as numbered tags; the rest of the 12 words and the passphrase are not
yet located, and the three locked pages are the obvious place they sit. I hold every password up to each
...
3. Page passwords on this site are case sensitive, but the case is set page by page and not
   ...
   lowercase.
4. The North branch was read page by page and ends on a static page with no further content.
   That read was looking for a hidden payload, so it is not evidence about the 12 words any
   more: the words turn out to be printed in the open (fact 10), and North needs re-reading.
...
10. Seed words 1 to 5 are `when`, `you`, `depart`, `find`, `mystery`, printed as numbered
    tags on the open East page headed "You find a message in a bottle... Could it be from
    her?", which also prints the next password, `witchoftheeast`, in clear. That page is the
    hunt's word channel in plain sight, and it puts The Wizard of Oz on the East branch.
...
| Circulating fragment completes with one missing word in the tail, under a Glimmer-derived passphrase | 7 tail positions, 943 checksum-valid mnemonics, 79 passphrases, 74,497 derivations | `tools/seedsearch.py --insert 6..12` | 0 match | yes: same self-test and bip_utils cross-check | 2026-09-03 |
...
1. **Re-walk every open page for numbered word tags, North first** (an hour or two, no new
   insight). The East branch prints seed words 1 to 5 as numbered tags on an open page. That
   is the word channel, and words 6 to 12 are probably printed the same way. Earlier searches
   looked for hidden payloads instead. North, written off as a dead end, is the first place to
   look: the circulating fragment's tail contains "whale", which is that branch's theme.
2. **Answer the East riddle in Oz, not Tolkien** (minutes). The same open page prints
   `witchoftheeast`, so Oz is on this branch as a fact. The riddle asks whether the images are
   a life passing or hallucinations, which is the argument the end of Oz turns on:
   `theresnoplacelikehome`, `noplacelikehome`, `kansas`. The Tolkien reading is the fallback,
   and its untried half is the vision rather than the refusal: `whiteshores`, `silverglass`.
3. **Read the West riddle's misspelling as the pointer it is** (minutes). "unbridaled"
...
4. **Read the South lock's "Name 6" as the first-season credit** (minutes). For all of season
